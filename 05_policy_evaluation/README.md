...
- A rollout **video per episode** (up to 10) is written under `$OUTPUT_DIR/eval_*/videos/<task>/eval_episode_*.mp4`
  — `lerobot-eval` always records these; `eval_info.json` lists every `video_path`.
